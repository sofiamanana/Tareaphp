    ...
    if($_POST){     
        foreach($playlist as $play){
            if(isset($_POST[$play['ID_Playlist']])){
                ...
                if(count($sen)==0){
                    $sql = "INSERT INTO Personas_SiguenP (ID_PERSONA,ID_Playlist) VALUES (?,?);";
                    $sentencia = $pdo->prepare($sql);
                    $sentencia->execute(array($id_persona,$play['ID_Playlist']));
                    ...
                    if($sen[0]['Cant_Seguidores']==NULL){
                        $sql = "UPDATE Playlist SET Cant_Seguidores = 0 WHERE ID_Playlist = ?;";
                        $sentencia = $pdo->prepare($sql);
                        $sentencia->execute(array($play['ID_Playlist']));
                    }

                    $sql = "UPDATE Playlist SET Cant_Seguidores = Cant_Seguidores+1 WHERE ID_Playlist = ?;";
                    $sentencia = $pdo->prepare($sql);
                    $sentencia->execute(array($play['ID_Playlist']));
